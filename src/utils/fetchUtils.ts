...
export const fetchSearch = async (text: string, page: number): Promise<any> => {
    return await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${api_key}&language=en-US&page=${page}&query=${text}`);
}

// value = true ---> ADD
// value = false --> REMOVE
export const addToFav = async (movieId: number, value: boolean): Promise<any> => {
    const body = {
        media_type: "movie",
        media_id: movieId,
        favorite: value
...
export const getFavList = async (): Promise<any> => {
    return await fetch(`https://api.themoviedb.org/3/account/acc/favorite?api_key=${api_key}&session_id=${session_id}`);
}

// value = true ---> ADD
// value = false --> REMOVE
export const addToWatchList = async (movieId: number, value: boolean): Promise<any> => {
    const body = {
        media_type: "movie",
        media_id: movieId,
        watchlist: value
